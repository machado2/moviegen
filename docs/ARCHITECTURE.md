...
| **Read-model de personagem** (derivar Character a partir dos assets) | `services/character.ts` (filme) e `comics/services/character.ts` (HQ) | ✅ extraído do `asset.ts`; resolvido o `listCharacters` duplicado (o leve virou `characterContext`) |
